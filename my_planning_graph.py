...
class PlanningGraph():
    ...
    def add_action_level(self, level):
		...
		# initialize a_nodes list
        a_nodes = []
        
		# for each action in all actions 
        for a in self.all_actions:
			# A-type (action) Planning Graph node 
            a_node = PgNode_a(a)
			# if a proposed a_node has prenodes that are a subset of the previous S level  
            if a_node.prenodes.issubset(self.s_levels[level]):
			    # action node is added     
                a_nodes.append(a_node)
				# action node MUST be connected to the S node instances in the appropriate s_level set
                for s_node in self.s_levels[level]:
                    # add children action node
    ...
    def add_literal_level(self, level):
		...
		# initialize s_nodes list empty set - a set is an unordered collection of objects and cannot have duplicate members
        s_nodes = set()
		
		# for each A node in the previous level
        for a_node in self.a_levels[level-1]:
		    # for each S node in effnodes that represent the effect produced by the action
            for s_node in a_node.effnodes:
			    # add S node to s_nodes list
                s_nodes.add(s_node)
				# add the A node to the parent sets of the S node
                s_node.parents.add(a_node)
				# add the new S node as children of the A node that could produce it
                a_node.children.add(s_node)
		# append literal to the new S level to the planning graph
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
	    ...
	    # for each effect in node_a1 action effect add
        for e in node_a1.action.effect_add:
			# if this action negates an action effect remove of the other node_a2 return true
            if e in node_a2.action.effect_rem:
                return True

		# for each effect in node_a2 action effect add
        for e in node_a2.action.effect_add:
		    # if this action negates an action effect remove of the other node_a1 return true
            if e in node_a1.action.effect_rem:
                return True

		# if one action allow an effect of the other return False
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		# for each effect in node_a1 action effect add
        for e in node_a1.action.effect_add:
		    # if this action negates a precondition of the other node_a1 return true
        ...
        for e in node_a2.action.effect_add:
            ...
            if e in node_a1.action.precond_neg:
                return True
        # for each effect in node_a2 action effect remove
        for e in node_a1.action.effect_rem:
		    # if this action negates a precondition of the other node_a2 return true
        ...
        for e in node_a2.action.effect_rem:
            ...
            if e in node_a1.action.precond_pos:
                return True
        # if the effect of one action allow a precondition of the other return false
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
		...
		# for each precondtion a1 in node a1 parents 
        for p_a1 in node_a1.parents:
		    # for each precondition a2 in node a2 parents
            for p_a2 in node_a2.parents:
				# return True if one of the precondition of one action is mutex with a precondition of the other action
                if p_a1.is_mutex(p_a2):
                    return True
		# return false if there aren't precondition of one action in mutex with a precondition of the other action			
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if node_s1.symbol == node_s2.symbol:
            if node_s1.is_pos != node_s2.is_pos:
			    # return True if one node is the negation of the other 
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
		...
		# for each precondition s1 in node s1 parents
        for p_s1 in node_s1.parents:
		    # for each precondition s2 in node s2 parents
            for p_s2 in node_s2.parents:
				# return False if there are actions that could achieve the two literals at the same time: no mutex precondition with node s1 and s2
                if not p_s1.is_mutex(p_s2):
                    return False
        # return True if there are no actions that could achieve the two literals at the same time
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
		# initialize the sum of the level costs of the individual goals
        level_sum = 0
        # TODO implement
        ...
        for g in self.problem.goal:
            # initialize found goal
            fg = False
			# for each depth in the range of lenght of s levels
            for d in range(len(self.s_levels)):
			    # for each state in s_levels depth
                for s in self.s_levels[d]:
				    # literal is expression state symbol
                    l = expr(s.symbol)
					# if state is not positive add literal
                    if not s.is_pos:
                        l = '~' + l
					# if literal is goal then turn found flag to true and add depth to level summ and exit from cicle
                    if l == g:
                        fg = True
                        level_sum += d						
                        break
				# if found flag is true then exit from cicle
                if fg:					
                    break
        # return the sum of the level costs of the individual goals 
        return level_sum 
